...
- Python requerido: **3.12** (estricto). Lo instala `uv`; no se usa conda.
- `.venv/` y `data/` están en `.gitignore`; `uv.lock` garantiza reproducibilidad
  de dependencias (incluido `torch==2.7.1+cu128`).
- Los modelos (`.pt`) NO se catalogan en Kedro: se leen/escriben por ruta con
  `portfolio_recsys.paths`.
- El horizonte `1y` se prepara pero no se entrena por defecto (efecto de borde
  del split de test).
...
- [ ] **`pr-report-pdf`** depende de Node.js/npx (dependencia externa a `uv`).
  Valorar documentar mejor el prerequisito o una alternativa en Python puro.
- [ ] Nomenclatura del target: en los artefactos la variable objetivo escalada es
  **`target_scaled`** y la versión sin escalar es **`target_log_return`** (la misma
  magnitud). Valorar unificar el nombre o dejar constancia del "raw" en el
  preprocessor para trazabilidad completa.
